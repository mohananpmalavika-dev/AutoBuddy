...
const logAlert = (message: string, data?: any) => {
  if (LOG_ALERTS && typeof console !== 'undefined') {
    console.log(`[TrafficAlerts] ${message}`, data || '');
  }
};
...
const normalizeAlert = (item: unknown, index: number): TrafficAlert | null => {
  ...
  return {
    ...
    isCrowdsourced: Boolean(row.isCrowdsourced ?? row.is_crowdsourced),
  };
};
...
const calculateAlertAge = (reportedTime: Date): string => {
  const now = Date.now();
  const ageMs = now - reportedTime.getTime();
